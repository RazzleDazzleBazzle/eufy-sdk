...
describe("describeCapabilities — enumeration of the live bound objects", () => {
   ...
   * and can be SET to the five confirmed settable. Published together so a caller shows the current
  ...
  it("offers what an action accepts, not everything its read reports", () => {
    ...
    expect(arming.actions.find((a) => a.name === "setMode")!.args![0].values).toEqual([0, 1, 2, 3, 63]);
  });
